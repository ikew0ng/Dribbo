...
public class StaggeredGridView extends ViewGroup {
	...
	public interface OnScrollListener {
		...
		public void onHeaderViewScroll(View headerView, float offset);

	}

	public interface OnRecycleEventListener {

		void OnRecycleView(View v);
	...
	private Drawable mTopBackgroundDrawable, mProjectionDrawable;
	...
	private boolean mFirstCached = false;

	protected OnRecycleEventListener mOnRecyleListener;

	protected OnScrollListener mOnScrollerListener;
	...
	public StaggeredGridView(Context context, AttributeSet attrs, int defStyle) {
		...
		mTopEdge = null;// new EdgeEffectCompat(context);
		mBottomEdge = null;// new EdgeEffectCompat(context);
	 ...
	 */
	public void setNumColumns(int colCount) {
	...
	public void setMinColumnWidth(int minColWidth) {
		...
		setNumColumns(COLUMN_COUNT_AUTO);
	}
	...
	private void dispatchOnScrollEvent() {

		if (mFirstPosition <= 1) {
			if (mItemTops != null) {
				mOnScrollerListener
						.onHeaderViewScroll(null, ((mFirstPosition == 0 ? 0
								: mHeaderViewHeight) + Math.abs(mItemTops[0]))
								/ (float) mHeaderViewHeight);
			}

		}
		if (mOnScrollerListener != null && mAdapter != null) {
			mOnScrollerListener.onScroll(this, mFirstPosition, getChildCount(),
					mAdapter.getCount());
...
//					+ "Using fallback spec of EXACTLY " + heightSize);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mFirstPosition <= 1 && mTopBackgroundDrawable != null) {
			int top = 0;
			if (mItemTops != null)
				top = mItemTops[0];
			if (mFirstPosition == 0 && this.getChildCount()>0) {
				mHeaderViewHeight = this.getChildAt(0).getMeasuredHeight();
			} else {
				top -= mHeaderViewHeight;
			}

			mTopBackgroundDrawable.setBounds(0, top, this.getWidth(),
					mTopBackgroundDrawable.getIntrinsicHeight() + top);
			mTopBackgroundDrawable.draw(canvas);
		}

		if (mProjectionDrawable != null) {
			// drawProjection(canvas);
		}

		if (mDrawLine) {
			float columnWidth = getWidth() / (float) mColCount;
			int h = getHeight();
			for (int i = 1; i < mColCount; i++) {
				canvas.drawLine(i * columnWidth, 0, i * columnWidth, h, mPaint);
			}

			drawDividers(canvas, columnWidth);
		}
	...
	private void drawProjection(Canvas canvas) {
		...
		// mProjectionDrawable.setBounds(
		// 0, 0,
		// 290, 310);
		for (int i = startPosition; i < this.getChildCount(); i++) {
			View child = this.getChildAt(i);
			mProjectionDrawable.setBounds(child.getRight()
					- mProjectionDrawable.getIntrinsicWidth(), child.getTop(),
					child.getRight(),
					child.getTop() + mProjectionDrawable.getIntrinsicHeight());

			mProjectionDrawable.draw(canvas);
		}
	}

	private void drawDividers(Canvas canvas, float columnWidth) {
		int startPosition = 0;// this.getFirstPosition();
		// if (startPosition <= 1)
		// startPosition = startPosition == 1 ? 1 : 2;
		// else
		// startPosition = 0;
		...
		for (int i = startPosition; i < this.getChildCount(); i++) {
			...
			int colIndex = ((LayoutParams) child.getLayoutParams()).position
					% mColCount;
			canvas.drawLine(colIndex * columnWidth, bottom, (colIndex + 1)
					* columnWidth, bottom + 1, mPaint);
			// bottom += step;
		}
	 ...
	 * 预加载下�?��View
	 */
	protected void preCacheDown() {
		// 优化
		int childCount = getChildCount();
		if (childCount > 0) {
			int lastPosition = ((LayoutParams) this.getChildAt(childCount - 1)
					.getLayoutParams()).position;
			for (int i = lastPosition + 1; (i < lastPosition + 1
					+ (lastPosition - mFirstPosition))
					&& i < mAdapter.getCount(); i++) {
				View newView = obtainView(i, null, false);
				if (newView != null) {
					mRecycler.addScrap(newView);
				}
			}
		}
	}
	...
	View obtainView(int position, View optScrap) {
		return obtainView(position, optScrap, true);
	}
	 ...
	 */
	View obtainView(int position, View optScrap, boolean reuse) {
		...
		if (reuse) {
			...
			final View scrap = optType == positionViewType ? optScrap
					: mRecycler.getScrapView(positionViewType, position);
			...
			view = mAdapter.getView(position, optScrap, this);
	...
	private class RecycleBin {
		...
		private int mMaxScrap = 12; // 优化，加大缓存容�?
		...
		public void addScrap(View v) {
			...
			if (scrap.size() < mMaxScrap) {
				...
				if (mOnRecyleListener != null)
					mOnRecyleListener.OnRecycleView(v);
			}
		}
		...
		public View getScrapView(int type, int position) {
	...
	public void setOnRecycleListner(
			OnRecycleEventListener onRecycleEventListener) {
		mOnRecyleListener = onRecycleEventListener;
	}

	public OnScrollListener getOnScrollerListener() {
		return mOnScrollerListener;
	}

	public void setOnScrollerListener(OnScrollListener mOnScrollerListener) {
	...
	private void changeScrollState(int state) {
		if (mOnScrollerListener != null && mLastTouchMode != state) {
			mOnScrollerListener.onScrollStateChanged(this, state);
		}
		mLastTouchMode = state;
	}

	public Drawable getTopBackgroundDrawable() {
		return mTopBackgroundDrawable;
	}

	public void setTopBackgroundDrawable(Drawable d) {
		mTopBackgroundDrawable = d;
		if (mTopBackgroundDrawable != null) {
			this.mTopBackgroundDrawable.setDither(false);
			mTopBackgroundDrawable.setFilterBitmap(false);
		}
	}

	public Drawable getProjectionDrawable() {
		return mProjectionDrawable;
	}

	public void setProjectionDrawable(Drawable d) {
		mProjectionDrawable = d;
		if (mProjectionDrawable != null) {
			mProjectionDrawable.setDither(false);
			mProjectionDrawable.setFilterBitmap(false);
		}
	}
